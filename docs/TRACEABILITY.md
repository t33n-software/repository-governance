...
| Canonical callers | VERIFIED | the four hash-pinned masters cover every shared line with the exact job names and grants; the canonical home pin is re-issued at the revision whose conformance action chains to the Go-native toolchain provisioning, so v4 tenants verify green; byte-identity with the home's own callers is proven by the contract-test set |
